...
| PG | task | #19(007完了)・#21(001完了)・#23(002完了)・#25(003完了)・#27(004完了)・#29(005完了)・#31(006完了)・#33(common完了) | front-intra-001〜007・common | frontend | feature/costman-2026-001-pg-front-intra-###-sp-002 | 手戻りsp-002対応として着手（2026-08-24）。front-intra-007（issue #19・PR #20）・front-intra-001（issue #21・PR #22）・front-intra-002（issue #23・PR #24）・front-intra-003（issue #25・PR #26）・front-intra-004（issue #27・PR #28）・front-intra-005（issue #29・PR #30）・front-intra-006（issue #31・PR #32）**全7機能ID完了**。いずれも`/reacter-code-gen`または既存実装照合→`/reacter-code-review`（観点台帳クラスタ別2パス検証）を実施。front-intra-004では**front-intra-005からの遷移state未受信という実装漏れ（機能バグ）を検出・修正**（useLocation対応）。front-intra-005では「プロ管取込」リンク表示条件の設計不整合（MUST）を検出・修正。front-intra-006ではCSV変換マクロダウンロードのキャッシュバスター未付与（COM-07）を修正、`accept`/`webkitdirectory`併用時のファイル種別フィルタ非機能とUI正本記述の不一致を検出。全機能ID横断の既存課題（COM-01・QLT-22）・並び替えフォームのRHF未使用（SS正本承認済み設計）・API件数上限未設定はADR-PG-1.mdにバックログ化。UI工程正本（docs/base-design/）のSS修正未追従・記述不一致はmeta.mdバックログ持ち越し管理表No.1〜12に記録。**追加で手戻りsp-003対応完了・マージ済み**（front-intra-004ソートバグ修正・部署列追加＝PR #34／共通レイアウトHeaderLayoutログアウトボタン・システムタイトル遷移・inert制御＝issue #33・PR #35。いずれも2026-08-27マージ）。sp-003で新たに検出した既存コード起因の指摘はNo.13〜16に記録。 |
...
| PT | task | #36(002着手) | front-intra-001〜007 | frontend | feature/costman-2026-001-pt-front-intra-### | front-intra-002着手（2026-08-27・issue #36）。他機能IDは未着手。front-intra-004向けに空ブランチ`-pt-front-intra-004`が存在するがコミットなし・issue未起票 |
...
| sp-003 | ①front-intra-004（工番別収支データ参照画面）のソート機能に、表示用の行非表示変換（`convertHiddenRow`）を並び替えの基準データ（`historyShushiData`）にも適用してしまうバグ（計画行・実算行のペア構造が壊れる）。②共通コンポーネント（`HeaderLayout`）は、SS工程の`old_docs`からの移行時にログアウトボタン・システムタイトルの遷移機能・メイン領域（寸法・inert制御）の仕様が正本設計書（`共通設計書.md`）へ転記されず、実装からも漏れていた | `/reacter-code-review`実施・ユーザーとの対話調査（2026-08-25） | SS（`コンポーネント仕様書_front-intra-004.md`・`共通設計書.md`の記述修正）・PG（front-intra-004実装修正・`HeaderLayout`実装追加。前者はスタック内で影響が閉じる、後者は複数機能ID（全画面）へ波及する共通部品） | front-intra-004: #27（既存issue継続使用・Reopen不要）／共通部品（HeaderLayout）: #33（新規起票） | **PG対応完了・マージ済み**（PR #34〔front-intra-004・2026-08-27〕・PR #35〔common HeaderLayout・2026-08-27〕）。両PRとも`reacter-code-review-report.md`・アドホックレビュー`reports/reacter_code_review/reacter_code_review-headerlayout-kobanbetsushushi-20260825_193534/`で新規実装分のMUST指摘（ソートバグ・QLT-11・CROSSCUT-a No.1/2）を全件解消し「PR提出可」。既存コード由来の指摘（COM-01・COM-05・列幅不一致5件・関数名不一致1件）はスコープ外としてバックログ持ち越し管理表No.13〜16に記録。詳細: `frontend/specs/costman-2026-001/rework/sp-003/rework-impact-report.md` |
...
| 5 | PG（front-intra-003・issue #25） | costman-frontend #25 | `docs/base-design/画面状態遷移図_front-intra-003.md`(L20)のコンポーネント名「MenuPage」→「Menu」に修正（SS工程の正本更新に未追従） | UI | Mai Tamura | 2026-08-24 | 未対応 |
| 6 | PG（front-intra-003・issue #25） | costman-frontend #25 | `docs/base-design/画面状態遷移図_front-intra-003.md`(L46-47)・`機能概要_front-intra-003.md`(L24,L42)のセッションタイムアウト監視の記述主体を「Menu画面自身」→「PrivateRoute配下のSessionTimeout共通部品」に修正（SS工程の正本更新に未追従） | UI | Mai Tamura | 2026-08-24 | 未対応 |
...
| 12 | PG（front-intra-006・issue #31） | costman-frontend #31 | `docs/base-design/ファイル定義書_front-intra-006.md`(L51)の「ファイル選択欄はtext/csvのみを選択候補として表示する」の記述を、`webkitdirectory`使用時のブラウザ仕様上の制約（accept属性によるフィルタが機能しない）を踏まえた実態に即した表現へ修正 | UI | Mai Tamura | 2026-08-25 | 未対応 |
| 13 | PG（front-intra-004・issue #27・手戻りsp-003・PR #34） | costman-frontend #27 | `useErrorHandling.ts`(L57-L58)でサーバ応答の`message`を加工せずそのままトースト表示している（COM-01・🚫禁止）。真因はBS-004 APIのレスポンス設計にある可能性があり、フロントエンド単体の修正では限定的な対応となる | UI/BS（要件・API設計の再検討） | Mai Tamura | 2026-08-27 | 未対応 |
| 14 | PG（front-intra-004・issue #27・手戻りsp-003・PR #34） | costman-frontend #27 | `KobanbetsuShushi.tsx`のグリッド列幅5件（shainNo:110→設計書100／shainNm:140→設計書120／yojitsuFlg:90+pinned→設計書60・pinnedなし／月別列:100→設計書80／total:110→設計書100）が`コンポーネント仕様書_front-intra-004.md`と不一致 | SS（設計書と実装値のどちらを正とするか再検討） | Mai Tamura | 2026-08-27 | 未対応 |
| 15 | PG（front-intra-004・issue #27・手戻りsp-003・PR #34） | costman-frontend #27 | `コンポーネント仕様書_front-intra-004.md`「グリッド列定義」記載の関数名`convertToYojitsuDisplay`が実装の`convertYojitsuFlgToValue`と不一致（CPC-2完全一致違反） | SS（設計書の関数名表記を実装に合わせて修正） | Mai Tamura | 2026-08-27 | 未対応 |
| 16 | PG（front-intra-004・issue #27・手戻りsp-003・PR #34） | costman-frontend #27 | 工番別収支検索API（bs-005）・工番検索API（bs-004）の取得件数上限未設定（COM-05）。No.9と同一事項のため対応時は統合すること | UI/SS | Mai Tamura | 2026-08-27 | 未対応（No.9と重複） |
