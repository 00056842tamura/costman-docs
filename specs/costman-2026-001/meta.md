...
| sp-003 | ①front-intra-004（工番別収支データ参照画面）のソート機能に、表示用の行非表示変換（`convertHiddenRow`）を並び替えの基準データ（`historyShushiData`）にも適用してしまうバグがあり、計画行・実算行のペア構造が壊れていた（PR #28マージ済み後に発覚）。②共通コンポーネント（`HeaderLayout`）は、SS工程の`old_docs`からの移行時にログアウトボタン・システムタイトルの遷移機能・メイン領域（寸法・inert制御）の仕様が正本設計書（`共通設計書.md`）へ転記されず、実装からも漏れていた | `/reacter-code-review`実施・ユーザーとの対話調査（2026-08-25） | SS（`コンポーネント仕様書_front-intra-004.md`・`共通設計書.md`の記述修正）・PG（front-intra-004実装修正・`HeaderLayout`実装追加。前者はスタック内で影響が閉じる、後者は複数機能ID（全画面）へ波及する共通部品） | front-intra-004: #27（Reopen不要・Open継続中）／共通部品（HeaderLayout）: 新規起票予定 | 対応中（2026-08-25）。詳細: `frontend/specs/costman-2026-001/rework/sp-003/rework-impact-report.md`（予定） |
