...
| PG | task | #19(007完了)・#21(001完了)・#23(002完了)・#25(003完了)・#27(004完了) | front-intra-001〜007 | frontend | feature/costman-2026-001-pg-front-intra-###-sp-002 | 手戻りsp-002対応として着手（2026-08-24）。front-intra-007（issue #19・PR #20）・front-intra-001（issue #21・PR #22）・front-intra-002（issue #23・PR #24）・front-intra-003（issue #25・PR #26）・front-intra-004（issue #27・PR #28）完了・レビュー待ち。いずれも`/reacter-code-gen`→`/reacter-code-review`（観点台帳クラスタ別2パス検証）を実施。front-intra-004では**front-intra-005からの遷移state未受信という実装漏れ（機能バグ）を検出・修正**（useLocation対応）。全機能ID横断の既存課題（COM-01・QLT-22）・並び替えフォームのRHF未使用（SS正本承認済み設計）・API件数上限未設定はADR-PG-1.mdにバックログ化。UI工程正本（docs/base-design/）のSS修正未追従はmeta.mdバックログ持ち越し管理表No.1〜9に記録。残り2機能ID（005〜006）へ展開中。事後修正: PR #11（`.env.development`/`.env.prod`のセッションタイムアウト設定値を仕様〔1時間・タイムアウト30秒前警告〕に修正、マージ済み2026-08-24） |
