#!/usr/bin/env bash
# check_backlog_gate.py 呼び出しラッパー（PreToolUse・matcher: Bash）。
#
# 既存フック（check_java.py 等）が使う `python3 ... 2>/dev/null || python ...` という
# フォールバック連結は、python3 が実行できた場合（＝コマンドが見つからず失敗するのではなく、
...
printf '%s' "$INPUT" | PYTHONUTF8=1 "$PYBIN" "$SCRIPT_DIR/check_backlog_gate.py"
exit $?
