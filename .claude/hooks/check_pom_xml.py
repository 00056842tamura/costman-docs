...
Write/Edit ツール実行前に pom.xml を検査し、禁止依存関係を検出してブロックします。
...
def main():
    try:
        ...
        if os.path.basename(file_path) != "pom.xml":
            return
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Lombok 依存関係
            if "<artifactId>lombok</artifactId>" in stripped:
                issues.append(
                    f"  [規約違反 L{i}] Lombok 依存関係禁止: {stripped}"
                    f" → <dependency> ブロックを削除すること"
                )
