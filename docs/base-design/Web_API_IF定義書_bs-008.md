...
> bs は別リポジトリ（`ctm-costman-backend-main`）で実装済み。本書は実装コード（`AdminTorikomiController`）との照合確認済みの内容に更新している（`old_docs/03.Web_API_IF定義書/Web_API_IF定義書(UPLOAD_SOFUKARI_DATA).md` からの移行を基に、実装調査で判明した相違点を反映）。
...
|---|---|
| 機能ID | bs-008 |
| API ID | UPLOAD_SOFUKARI_DATA |
| API名 | ソフ仮データアップロードAPI |
| API説明 | 管理者用データ取り込み画面の「ソフ仮データ CSV取り込み」から呼び出す |
| Method | PUT |
| Path | `/api/upload-sofukaridata`（ベースURL: `http://localhost:8080`。`/api`プレフィックス必須） |
...
|---|---|---|---|---|---|---|
| 1 | 年度 | `nendo` | string | 4 / 4 | ○ | 半角数字のみ。選択肢は当年度を含む3年度分 |
| 2 | ソフ仮データファイル | `sofukariData` | file | - | ○ | 選択された1件目のファイルのみを送信。フォームデータ形式に変換 |
...
```json
{ "detail": null, "errorCode": "bs.error.business.SofukariDataNendoDifference", "message": "ファイル内の年度と選択した年度が一致しません。" }
```
